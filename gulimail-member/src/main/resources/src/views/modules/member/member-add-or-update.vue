<template>
  <el-dialog
    ...
    <el-form-item label="微博社交账户uid（应该建立关联表）" prop="weiboUid">
      <el-input v-model="dataForm.weiboUid" placeholder="微博社交账户uid（应该建立关联表）"></el-input>
    </el-form-item>
    <el-form-item label="访问令牌" prop="accessToken">
      <el-input v-model="dataForm.accessToken" placeholder="访问令牌"></el-input>
    </el-form-item>
    <el-form-item label="访问令牌的过期时间" prop="expiresIn">
      <el-input v-model="dataForm.expiresIn" placeholder="访问令牌的过期时间"></el-input>
    </el-form-item>
    </el-form>
    ...
    data () {
      return {
        ...
        dataRule: {
          ...
          weiboUid: [
            { required: true, message: '微博社交账户uid（应该建立关联表）不能为空', trigger: 'blur' }
          ],
          accessToken: [
            { required: true, message: '访问令牌不能为空', trigger: 'blur' }
          ],
          expiresIn: [
            { required: true, message: '访问令牌的过期时间不能为空', trigger: 'blur' }
          ]
        }
      }
    },
          ...
          if (this.dataForm.id) {
            this.$http({
              ...
              if (data && data.code === 0) {
                ...
                this.dataForm.weiboUid = data.member.weiboUid
          ...
          if (valid) {
            this.$http({
              ...
              data: this.$http.adornData({
                ...
                'weiboUid': this.dataForm.weiboUid,
